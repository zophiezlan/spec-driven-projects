...
if [ ${#BRANCH_NAME} -gt $MAX_BRANCH_LENGTH ]; then
    ...
    >&2 echo "[nuaa] Warning: Branch name exceeded GitHub's 244-byte limit"
    >&2 echo "[nuaa] Original: $ORIGINAL_BRANCH_NAME (${#ORIGINAL_BRANCH_NAME} bytes)"
    >&2 echo "[nuaa] Truncated to: $BRANCH_NAME (${#BRANCH_NAME} bytes)"
fi
